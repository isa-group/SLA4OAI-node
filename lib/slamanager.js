...
SlaManager.prototype.initialize = function (app, configObj, callback) {
    ...
    loadSla4OaiDoc(configObj.sla4oai, (err, sla) => {
        ...
        if (!doc.infrastructure || !doc.infrastructure.supervisor) {
            return callback(null, new Error('Invalid supervisorConnection, missing infrastructure.supervisor property'));
        }
...
function loadSla4OaiDoc(url, callback) {
    ...
    if (url.indexOf("http") == -1) {
        ...
        request.get(url, {
            ...
            if (!slaObject.context || slaObject.context.type != "plans") {
                callback(new Error('Agreement file is not valid.'), null);
            } else if (!slaObject.infrastructure || !slaObject.infrastructure.supervisor || !slaObject.infrastructure.monitor) {
