...
ScopeResolver.prototype.configure = function (options) {
  ...
  this._oauthConfig = options.oauthConfig;
  return this;
};
...
function resolveScope(req, res, next) {
  if (req.query.apikey) {
    ...
    getAccountName(token, function (err, accountName) {
      if (err) {
        return next(err);
      }
 ...
 * @api private
 */
function getAccountName(token, callback) {
  switch (scopeResolver._oauthProvider) {
    case "Google":
      return getGoogleAccountName(token, callback);
    default:
      return callback(new SlaError('Invalid oauthProvider: ' + scopeResolver._oauthProvider));
  }
}

function getGoogleAccountName(token, callback) {
  var strategy = new GoogleStrategy({
    clientID: scopeResolver._oauthConfig.clientId,
    clientSecret: scopeResolver._oauthConfig.clientSecret,
    callbackURL: scopeResolver._oauthConfig.callbackURL
  ...
  strategy.userProfile(token, function (err, profile) {
    ...
    if (profile && profile.emails && profile.emails.length > 0 && profile.emails[0].value) {
      ...
      return callback(new SlaError('Can not retrive profile form OAuth Provider'));
    }
  });
}
