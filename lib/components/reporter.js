...
function reportMetrics(req, res) {
  reporter.report(buildMetricsPostBody(req, res), function (err, result) {
    if (err) {
      console.log("Error while sending metrics: " + err.message);
    }

    console.log("Metrics sent: " + result);
  });
}
...
function buildMetricsPostBody(req, res) {
  return {
